...
def plot(dataset,model,agerange,corr):
    ...
    for m in metric_titles:
        ...
        if(ax_i==0):
            ...
            lab = "models (N = 20,043)" #lower
            # upper #lab = "models (N = 12,796)"
            label = "Age range "+lab
        ...
        corr_line_label = ""
        if(ax_i==0):
            line_lab = "model (N = %.0f)" % results_dict["all"]["n"]
    ...
    fig.savefig(f"../plots/additional_plots/{dataset}_metric_results_corr_{corr}{model_name}_age_{agerange}.pdf")
